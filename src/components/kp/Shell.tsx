...
const tabs = [
  { to: "/home", label: "Utama", icon: Home },
  { to: "/send", label: "Hantar", icon: Send },
  { to: "/qr", label: "QR", icon: QrCode },
  { to: "/transactions", label: "Sejarah", icon: Receipt },
...
export function ConnectivityPill() {
  ...
  const map = {
    online: { Icon: Wifi, text: "Dalam Talian", cls: "bg-success/15 text-success" },
    weak: { Icon: SignalLow, text: "Isyarat Lemah", cls: "bg-warning/20 text-warning-foreground" },
    offline: { Icon: WifiOff, text: "Luar Talian", cls: "bg-destructive/15 text-destructive" },
  } as const;
  ...
  return (
    <button
      ...
      aria-label="Tukar status sambungan (demo)"
    >
    ...
    <header className="sticky top-0 z-20 bg-background/95 backdrop-blur border-b border-border">
      <div className="flex items-center justify-between px-4 h-14">
        <div className="flex items-center gap-2 min-w-0">
          {back ? (
            <Link to={back} className="text-primary font-semibold text-sm">‹ Kembali</Link>
          ) : (
        ...
        <div className="flex items-center gap-2">
          ...
          <Link to="/notifications" aria-label="Notifikasi" className="p-2 rounded-full hover:bg-muted">

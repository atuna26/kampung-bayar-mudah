    ...
    <Shell title="Sejarah Transaksi">
      {pendingCount > 0 && (
        <div className="mb-3 rounded-2xl bg-warning/15 p-3 flex items-center gap-3">
          ...
          <p className="flex-1 text-sm"><b>{pendingCount} transaksi</b> menunggu disegerak</p>
          <button disabled={conn==="offline"} onClick={()=>store.syncPending()} className="text-xs font-bold px-3 py-1.5 rounded-full bg-primary text-primary-foreground disabled:opacity-50">Segerak</button>
        </div>
      )}
      <div className="flex gap-2 mb-4 overflow-x-auto -mx-4 px-4 pb-1">
        {[{k:"all",l:"Semua"},{k:"sent",l:"Hantar"},{k:"received",l:"Terima"},{k:"merchant",l:"Bayaran QR"}].map(f=>(
      ...
      <div className="rounded-2xl bg-card border border-border divide-y divide-border overflow-hidden">
        ...
        {filtered.length===0 && <p className="p-8 text-center text-sm text-muted-foreground">Tiada transaksi</p>}
      </div>
    </Shell>
  );
}

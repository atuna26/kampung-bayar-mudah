...
function Transactions() {
  ...
  return (
    <Shell title={t("Transaction History")}>
      {pendingCount > 0 && (
        <div className="mb-3 rounded-2xl bg-warning/15 p-3 flex items-center gap-3">
          {conn==="offline" ? <WifiOff className="h-5 w-5 text-warning-foreground"/> : <RefreshCw className="h-5 w-5 text-warning-foreground"/>}
          <p className="flex-1 text-sm">
            <b>{pendingCount} {t("Transactions").toLowerCase()}</b> {conn==="offline" ? t("Waiting for network").toLowerCase() : "waiting to sync"}
          </p>
          <button disabled={conn==="offline"} onClick={()=>store.syncPending()} className="text-xs font-bold px-3 py-1.5 rounded-full bg-primary text-primary-foreground disabled:opacity-50">{t("Sync")}</button>
        </div>
      )}
      <div className="flex gap-2 mb-4 overflow-x-auto -mx-4 px-4 pb-1">
        {[{k:"all",l:t("All")},{k:"sent",l:t("Send")},{k:"received",l:t("Receive")},{k:"merchant",l:t("QR Payments")}].map(f=>(
